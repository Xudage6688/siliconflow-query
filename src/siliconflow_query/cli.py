...
def update(
    ...
    for m in free_models:
        ...
        # 解析上下文长度
        ctx_str = str(m.get("context_length", "0"))
        if "K" in ctx_str:
            context_length = int(ctx_str.replace("K", "").replace(",", "")) * 1024
        else:
            context_length = int(ctx_str.replace(",", "") or "0")

        db_data["models"].append({
            "id": model_id,
            "name": m.get("name", model_id.split("/")[-1] if "/" in model_id else ""),
            "provider": m.get("provider", model_id.split("/")[0] if "/" in model_id else ""),
            "provider_cn": m.get("provider", ""),
            "model_type": m.get("model_type", model_type),
            "context_length": context_length,
            "max_output": 4096,
            "parameters": parameters,
            "description": m.get("description", ""),
            "capabilities": [model_type] if model_type in ["chat", "embedding", "reranker"] else ["chat"],
            "pricing_tier": "free",
            "source": "scraped",
        })
